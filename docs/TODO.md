...
- [x] **v2.8.0**: Automated Micro-Benchmark & Performance Regression Synthesizer across Rust (Criterion), TypeScript (tinybench), Python (timeit), Go (testing.B), Java (JMH), and C# (BenchmarkDotNet) (`cddm_core::extract::bench_generator`, CLI `cddm extract --bench`, MCP `generate_benchmarks`, WebUI Benchmark previewer, TUI `[b]` badge).
