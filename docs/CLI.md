...
| **`cddm prune`**                    | `cddm prune [OPTIONS] [DIRECTORY]`            | Automatically prune unreachable dead clone clusters and unreferenced code         | `--dry-run`, `--safe-only`, `--threshold`, `--min-tokens`, `--format`, `--languages`                                                                       |
...
| **`cddm hub`**                      | `cddm hub <init                               | scan                                                                              | extract                                                                                                                                                    | sync> [OPTIONS]`                                                                | Manage and scan multi-repository Organization Federation Hub (.cddmhub.toml)  | `--config`, `--targets`, `--cluster`, `--pkg-name`, `--pkg-type`, `--target-dir`, `--remote`, `--salt` |
