...
3. **Gitea Push (Sole Push Target)**: Push to `origin` (Gitea: `https://git.gt-web-dev.com/gt-dev/cddm.git`) without bypassing Git hooks (`--no-verify` is forbidden), using strictly ONE canonical branch name (`feat/issue-<num>-<desc>`). Do NOT push manually to `github`—Gitea automatically replicates all branches, commits, and tags to GitHub via server-side push mirror (`sync_on_commit: true`).
4. **Primary Pull Request**: Open the primary Pull Request via Forge MCP (`gitea_create_pull_request` or `forge pr create`) merging into `main`. Include `Fixes #<id>` in the description and assign the target milestone.
5. **Verify CI Quality Gate**: Monitor and verify CI gate status using Forge MCP (`gitea_wait_for_ci_gate` or `gitea_get_commit_statuses`). Never bypass CI gates (`--no-verify` or manual fast-forwards are strictly prohibited).
6. **API-Driven Merge**: Conclude the PR merge via Forge MCP (`gitea_merge_pull_request` or `forge pr merge <id> --delete-branch`) to register the PR as `merged: true`, auto-delete the remote branch, and automatically close the linked issue.
7. **Milestone Release & Version Enforcement**: Workspace versions are determined strictly by milestones. When the last issue in a milestone is closed (100% milestone completion), execute `bun scripts/milestone-release.ts` (or `vp run release:milestone`) to automagically sync manifests, tag `vX.Y.Z`, push to Gitea `origin` (auto-mirrored to GitHub), close the milestone, and publish the release.
