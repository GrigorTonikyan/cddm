...
fn write_file_with_parent_dir(abs_path: &Path, content: &str, label: &str) -> Result<(), String> {
    if let Some(parent) = abs_path.parent() {
        fs::create_dir_all(parent)
            .map_err(|e| format!("Failed to create directory '{}': {}", parent.display(), e))?;
    }
    fs::write(abs_path, content)
        .map_err(|e| format!("Failed to write {label} '{}': {}", abs_path.display(), e))
}
...
pub fn apply_extraction_to_workspace(
    ...
    for gen_file in &result.generated_files {
        ...
        write_file_with_parent_dir(&abs_path, &gen_file.content, "file")?;
    ...
    for test_file in &result.test_files {
        ...
        write_file_with_parent_dir(&abs_path, &test_file.content, "test file")?;
        total_files_written += 1;
    }
    ...
    for bench_file in &result.benchmark_files {
        ...
        write_file_with_parent_dir(&abs_path, &bench_file.content, "benchmark file")?;
        total_files_written += 1;
    }
    ...
    for update in &result.manifest_updates {
        ...
        write_file_with_parent_dir(&abs_path, &update.updated_content, "manifest")?;
        total_files_written += 1;
    }
