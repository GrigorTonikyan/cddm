...
- **test**: resilient milestone release engine test when no open issues exist (`b96c501`)

### Tooling & Maintenance

- **toolchain**: migrate workspace to rustc 1.100.0-nightly and update deps (`dd0f4aa`)

## [4.0.0] - 2026-09-08

### Features

- **hub**: federation hub remote peering protocol and distributed cache sync (`16217ea`)

### Performance Improvements

- **simd**: implement AVX-512 acceleration and parallel clone merger (`f3efd71`)
- **neural**: implement SQ8 vector quantization and HNSW search (Fixes #8) (`04f45fe`)
...
- **dead-code**: cross-repository dead code & call-graph reachability tracer (#27) (`6ecc4d2`)
- **dead-code**: implement cross-repository reachability tracer (`1ed2e32`)
- **core**: automated dead clone cluster pruning & safe deletion synthesizer (#134) (`f1b3d6f`)
...
- **ci**: configure memory-safe Cargo build limits and add E2E quality validation (#135) (`2e9e937`)
- **ci**: ensure CLI binary is compiled before dogfooding scan (`e9929a6`)
- **ci**: configure memory-safe Cargo build limits and add E2E quality validation (`dbc66eb`)
