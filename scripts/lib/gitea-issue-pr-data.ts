 ...
 * AUTO-GENERATED BY migrate-docs-to-issues.ts
 */
...
export const SEED_ISSUES: IssueDef[] = [
  {
    title: "[FR-1] Polyglot Tokenization Engine",
    ...
    milestoneTitle: "v1.9.0 - Polyglot Parity & Distribution Release",
    labels: ["feat: core-ast", "documentation"],
    closed: true,
  },
  {
    title: "[FR-2] Winnowing Fingerprint Engine",
  ...
  {
    title: "[FR-3] Clone Detection Pipeline",
  ...
  {
    title: "[FR-5] Git Blame Annotation",
  ...
  {
    title: "[FR-6] Tree-sitter AST Module",
  ...
  {
    title: "[FR-8] Language Server Protocol (LSP 3.17) Engine",
    body: "### Description\n\n| ID     | Requirement                  | Acceptance Criteria                                                                       | Status      |\n| :----- | :--------------------------- | :---------------------------------------------------------------------------------------- | :---------- |\n| FR-8.1 | Stdio LSP Daemon             | `cddm lsp` daemon handles standard LSP 3.17 JSON-RPC over stdio                           | Implemented |\n| FR-8.2 | Real-Time Diagnostics        | Emits `textDocument/publishDiagnostics` with clone ranges and `relatedInformation` links  | Implemented |\n| FR-8.3 | Quick-Fix Code Actions       | Emits `textDocument/codeAction` for instant function extraction and suppression insertion | Implemented |\n| FR-8.4 | Cross-Clone Navigation Hover | Emits `textDocument/hover` and `textDocument/definition` for counterpart clone jump       | Implemented |",
    milestoneTitle: "v1.9.0 - Polyglot Parity & Distribution Release",
    labels: ["feat: core-ast", "documentation"],
    closed: true,
  },
  {
    title: "[FR-9] AST-Native Refactor Surgeon & Autonomous Healing",
    body: "### Description\n\n| ID     | Requirement                 | Acceptance Criteria                                                                  | Status      |\n| :----- | :-------------------------- | :----------------------------------------------------------------------------------- | :---------- |\n| FR-9.1 | Tree-sitter CST Rewriting   | Inferred parameter typing and AST node substitution in `cddm-core::ast::rewriter`    | Implemented |\n| FR-9.2 | Closed-Loop Test Verifier   | Automated test runner executes test suite and verifies behavioral zero-regression    | Implemented |\n| FR-9.3 | Autonomous Self-Healing     | Multi-provider LLM healer repairs broken AST patches with compiler/test feedback     | Implemented |\n| FR-9.4 | Transactional Git Branching | Safely creates isolated Git branch for refactored code without dirtying working tree | Implemented |",
    milestoneTitle: "v1.9.0 - Polyglot Parity & Distribution Release",
    labels: ["feat: core-ast", "documentation"],
    closed: true,
  },
  {
    title: "[FR-10] Architectural Boundary Policy Engine",
    body: "### Description\n\n| ID      | Requirement              | Acceptance Criteria                                                                     | Status      |\n| :------ | :----------------------- | :-------------------------------------------------------------------------------------- | :---------- |\n| FR-10.1 | `.cddmrules.toml` Parser | Enforces layer boundaries, zero-duplication zones, and token thresholds                 | Implemented |\n| FR-10.2 | SARIF Violation Mapping  | Maps policy breaches to standard `CDDM_BOUNDARY`, `CDDM_ZERO_DUP`, `CDDM_LIMIT` rules   | Implemented |\n| FR-10.3 | Interactive Rules Studio | `PolicyRulesModal.tsx` provides live TOML editor and interactive active rules inspector | Implemented |",
    milestoneTitle: "v1.9.0 - Polyglot Parity & Distribution Release",
    labels: ["feat: core-ast", "documentation"],
    closed: true,
  },
  {
    title: "[FR-11] Deep Semantic Graph Matching & Neural Vector Embeddings",
    body: "### Description\n\n| ID      | Requirement                | Acceptance Criteria                                                                     | Status      |\n| :------ | :------------------------- | :-------------------------------------------------------------------------------------- | :---------- |\n| FR-11.1 | CFG & PDG Graph Extraction | Extracts Control Flow Graphs and Program Dependence def-use graphs from AST             | Implemented |\n| FR-11.2 | Weisfeiler-Lehman Kernels  | Structural graph kernel isomorphism hashing for Type-4 semantic clones                  | Implemented |\n| FR-11.3 | Subword Vector Embeddings  | Dense vector representations and cosine similarity for polyglot algorithmic equivalence | Implemented |",
    milestoneTitle: "v1.9.0 - Polyglot Parity & Distribution Release",
    labels: ["feat: core-ast", "documentation"],
    closed: true,
  },
  {
    title: "[FR-12] Interactive Terminal UI Studio (TUI)",
    body: "### Description\n\n| ID      | Requirement                | Acceptance Criteria                                                                 | Status      |\n| :------ | :------------------------- | :---------------------------------------------------------------------------------- | :---------- |\n| FR-12.1 | Crossterm/Ratatui Studio   | `cddm tui` launches terminal dashboard across 12 tabs with full keyboard navigation | Implemented |\n| FR-12.2 | Split Diff Viewer in TUI   | Interactive side-by-side split diff visualizer with synchronized scrolling          | Implemented |\n| FR-12.3 | Live Watch Synchronization | Re-scans and updates TUI in real time when files change                             | Implemented |\n\n---",
    milestoneTitle: "v1.9.0 - Polyglot Parity & Distribution Release",
    labels: ["feat: core-ast", "documentation"],
    closed: true,
  },
  {
    title: "[EP-01] SARIF Standard Reporter & GitHub Code Scanning Integration",
    body: "- **Target Milestone**: `v0.2.0`\n- **Component**: `crates/cddm-cli`, `crates/cddm-core`\n- **Priority**: `High`\n- **Status**: `Completed (v0.2.0)`\n\n#### Problem Statement\n\nCI/CD workflows currently support JSON, Markdown, and Console table formats. GitHub Code Scanning and IDE diagnostic viewers require standard SARIF (Static Analysis Results Interchange Format, OASIS Standard) to render inline warnings on pull requests.\n\n#### Specification & Architecture\n\n1. Add `OutputFormat::Sarif` variant to CLI options in `crates/cddm-cli/src/main.rs`.\n2. Map each `ClonePair` to a SARIF `result` object:\n   - Rule ID: `CDDM001` (Exact Clone), `CDDM002` (Renamed Clone), `CDDM003` (Near-Miss Clone).\n   - Level: `warning` (or `error` if duplication exceeds `--fail-threshold`).\n   - Locations: Physical file paths and 1-based start/end line numbers with secondary locations pointing to matched counterpart files.\n3. Emit compliant SARIF 2.1.0 JSON schema.\n\n#### Acceptance Criteria\n\n- `cddm scan . --format sarif` emits valid SARIF 2.1.0 validated against official OASIS JSON Schema.\n- Uploading SARIF file to GitHub Code Scanning via `github/codeql-action/upload-sarif@v3` surfaces inline PR annotations.\n\n---",
    milestoneTitle: "v3.2.0 - High-Performance Parser Recycling & AST Query Memoization",
    labels: ["feat: architecture", "priority: medium"],
    closed: true,
  },
  {
    title: "[EP-02] Advanced MCP Agentic Toolset & Resource Protocol",
  ...
  {
    title: "[EP-07] Automated Refactoring & Patch Synthesis Engine (`cddm refactor`)",
  ...
  {
    title: "[EP-08] Differential Scanning & Branch Comparison (`cddm diff`)",
  ...
  {
    title: "[EP-09] Polyglot Language Registry Expansion",
  ...
  {
    title: "[EP-19] Turnkey PR/MR Markdown Quality Gate Comment Generator",
  ...
  {
    title: "[EP-26] Monorepo Multi-Workspace Scanner & Distributed Cache Archive (.cddmpack)",
  ...
  },
  {
    ...
    body: "- **Target Milestone**: `v2.3.0`\n- **Component**: `crates/cddm-core`, `crates/cddm-cli`, `crates/cddm-mcp`, `webui`\n- **Priority**: `High`\n- **Status**: `Completed (v2.3.0)`\n\n#### Problem Statement\n\nPolyglot enterprise codebases frequently rewrite or duplicate algorithms, validation routines, and domain business logic across different programming languages (e.g. TypeScript frontend matching Rust backend, Python data pipeline matching Go microservice). Detecting these cross-language semantic duplicates requires polyglot CFG/PDG graph isomorphism combined with subword vector embeddings.\n\n#### Specification & Architecture\n\n1. **Subword Vector Embeddings & Cosine Similarity (`cddm-core::semantic_graph::embedding`)**:\n   - Sparse normalized term-frequency vectors combined with subword 3-grams and canonical operational tokens (`decl_var`, `ctrl_loop`, `op_add`, `lit_num`).\n   - Fast cosine dot-product similarity computation ($0.0 \\dots 1.0$).\n2. **Polyglot Graph Isomorphism & Def-Use Slots (`cddm-core::semantic_graph::cfg`, `pdg`)**:\n   - Language-agnostic CFG function boundary detection across 24 supported languages.\n   - Slot-normalized variable def-use chain mapping ($v_0, v_1, \\dots$).\n   - Weisfeiler-Lehman graph coloring kernel.\n3. **Unified Hybrid Similarity Model (`compute_hybrid_similarity`)**:\n   - Combines structural graph isomorphism ($S_{\\text{graph}}$) and subword token embedding ($S_{\\text{token}}$):\n     $$S_{\\text{hybrid}} = \\alpha \\cdot S_{\\text{graph}} + (1 - \\alpha) \\cdot S_{\\text{token}}$$\n4. **Surfaces & Integrations**:\n   - **CLI**: `cddm semantic [DIR] [--threshold <N>] [--format console|json|markdown]` and `--cross-language` scanning flags for `scan` and `diff`.\n   - **Axum REST API**: `POST /api/semantic/scan` and dual-language `POST /api/semantic-graph`.\n   - **MCP Server**: `cddm_scan_cross_language` tool, `cross_language_audit` prompt, and `cddm://workspace/cross_language_clones` resource.\n   - **WebUI Studio**: Interactive Cross-Language Explorer tab, dual-language Polyglot Sandbox selectors, and `[Polyglot]` clone badges.\n\n---",
    milestoneTitle: "v3.2.0 - High-Performance Parser Recycling & AST Query Memoization",
    labels: ["feat: architecture", "priority: medium"],
    closed: true,
  },
  {
    title: "[EP-31] Automated Shared Module & Crate Extraction",
    body: "- **Target Milestone**: `v2.4.0`\n- **Component**: `crates/cddm-core`, `crates/cddm-cli`, `crates/cddm-mcp`, `webui`\n- **Priority**: `High`\n- **Status**: `Completed (v2.4.0)`\n\n#### Problem Statement\n\nRefactoring duplicate code across packages or crates currently stops at generating a patch. Developers need automated packaging that creates a standalone shared crate or module, updates workspace root manifests (`Cargo.toml`, `package.json`), adds inter-package dependencies, and rewrites all caller occurrences with imports.\n\n#### Specification & Architecture\n\n1. **Extraction Engine (`crates/cddm-core/src/extract/`)**:\n   - Manifest updaters for Cargo workspaces, npm/pnpm/yarn packages, pyproject.toml, and go.mod.\n   - Target crate/module boilerplate generators with public signature synthesis.\n   - Caller AST and text rewriters with import injection and callsite substitutions.\n   - Dry-run simulation and transactional disk execution.\n2. **CLI Subcommand (`cddm extract`)**:\n   - Flags: `--pair`, `--cluster`, `--target`, `--fn-name`, `--crate-type`, `--dry-run`, `--apply`.\n3. **Axum REST API & MCP Server**:\n   - Endpoints: `POST /api/extract/preview`, `POST /api/extract/apply`.\n   - MCP tool: `cddm_extract_shared_module`.\n4. **WebUI Studio Visualizer**:\n   - `ExtractModuleTab.tsx` in `RefactorSandboxModal.tsx` for visual inspection of generated files, manifest diffs, and caller rewrites.\n\n---",
    milestoneTitle: "v3.2.0 - High-Performance Parser Recycling & AST Query Memoization",
    labels: ["feat: architecture", "priority: medium"],
    closed: true,
  },
  {
    title: "[EP-32] TUI Studio Interactive Terminal Dashboard",
    body: "- **Target Milestone**: `v2.5.0`\n- **Component**: `crates/cddm-cli`, `crates/cddm-core`\n- **Priority**: `High`\n- **Status**: `Completed (v2.5.0)`\n\n#### Problem Statement\n\nTerminal power-users, remote SSH developers, and CI/CD engineers require a lightweight, keyboard-driven dashboard for real-time deduplication inspection without running a web browser or Node.js server.\n\n#### Specification & Architecture\n\n1. **Ratatui & Crossterm TUI Engine (`crates/cddm-cli/src/tui/`)**:\n   - 8 interactive tabs: Scan, Pairs, Clusters, Treemap, Timeline, Rules, Semantic, and Health.\n   - Side-by-side synchronized split diff viewer and refactor patch preview with scrolling.\n2. **CLI Subcommand (`cddm tui`)**:\n   - Zero-dependency terminal UI launched directly via `cddm tui [DIR]`.\n3. **Cross-Interface Feature Parity**:\n   - Governed by `.agents/rules/interface-feature-parity.md` and verified in `scripts/check-feature-parity.ts`.\n\n---",
    milestoneTitle: "v3.2.0 - High-Performance Parser Recycling & AST Query Memoization",
    labels: ["feat: architecture", "priority: medium"],
    closed: true,
  },
  {
    title: "[EP-33] Polyglot AST-Native Rewriters & Multi-Language Shared Module Extraction",
    body: "- **Target Milestone**: `v2.6.0`\n- **Component**: `crates/cddm-core`, `crates/cddm-cli`, `crates/cddm-mcp`, `webui`\n- **Priority**: `High`\n- **Status**: `Completed (v2.6.0)`\n\n#### Problem Statement\n\nExtraction of shared modules must support all major programming languages beyond Rust and TypeScript, updating language-specific workspace manifests, generating typed signatures, and rewriting caller call sites with correct import statements.\n\n#### Specification & Architecture\n\n1. **Multi-Language Manifest Updaters (`crates/cddm-core/src/extract/manifest/`)**:\n   - Support for Python (`pyproject.toml`), Go (`go.mod`, `go.work`), Java (`pom.xml`, `build.gradle`), and C# (`.csproj`).\n2. **Polyglot Boilerplate & Typing**:\n   - Typed function signature generation and class encapsulation across supported languages.\n   - Return type and multi-return inference engine in `crates/cddm-core/src/ast/type_infer/`.\n3. **CST Replacement & Caller Rewriting**:\n   - Import statement resolution and CST replacement in `crates/cddm-core/src/ast/import_resolver.rs` and `rewriter.rs`.\n\n---",
    milestoneTitle: "v3.2.0 - High-Performance Parser Recycling & AST Query Memoization",
    labels: ["feat: architecture", "priority: medium"],
    closed: true,
  },
  {
    title: "[EP-34] Automated Polyglot Unit Test Synthesizer & Behavioral Equivalence Verifier",
    body: '- **Target Milestone**: `v2.7.0`\n- **Component**: `crates/cddm-core`, `crates/cddm-cli`, `crates/cddm-mcp`, `webui`\n- **Priority**: `High`\n- **Status**: `Completed (v2.7.0)`\n\n#### Problem Statement\n\nWhen extracting duplicate code across codebases into shared helper modules or crates, engineers must ensure behavioral equivalence by generating idiomatic unit tests for the extracted helpers with sample arguments extracted from duplicate occurrences.\n\n#### Specification & Architecture\n\n1. **Polyglot Test Synthesizer (`crates/cddm-core/src/extract/test_generator.rs`)**:\n   - Generates idiomatic test files for Rust (`tests/*_test.rs`), TypeScript/JavaScript (`*.test.ts`), Python (`test_*.py`), Go (`*_test.go`), Java (`*Test.java`), and C# (`*Tests.cs`).\n   - Extracts sample arguments from duplicate occurrences to instantiate realistic test invocations.\n2. **4-Pillar Cross-Interface Integration**:\n   - CLI: `--generate-tests` flag for `cddm extract`.\n   - MCP: `generate_tests` parameter on `cddm_extract_shared_module`.\n   - WebUI: "Synthesize Unit Tests" checkbox and test file preview viewer in `ExtractModuleTab.tsx`.\n   - TUI: `[t] Synthesize Unit Tests` badge in extraction operations.\n\n---',
    milestoneTitle: "v3.2.0 - High-Performance Parser Recycling & AST Query Memoization",
    labels: ["feat: architecture", "priority: medium"],
    closed: true,
  },
  {
    title: "[EP-36] Automated Micro-Benchmark & Performance Regression Synthesizer",
    body: "- **Target Milestone**: `v2.8.0`\n- **Component**: `crates/cddm-core`, `crates/cddm-cli`, `crates/cddm-mcp`, `webui`\n- **Priority**: `Medium`\n- **Status**: `Completed (v2.8.0)`\n\n#### Problem Statement\n\nRefactoring duplicate code into shared helpers can introduce call overhead or memory regressions. Automated micro-benchmarking verifies that extracted modules meet performance baselines.\n\n#### Specification & Architecture\n\n1. Generate Criterion (Rust), tinybench (TypeScript), timeit (Python), testing.B (Go), JMH (Java), and BenchmarkDotNet (C#) suites in `crates/cddm-core/src/extract/bench_generator.rs`.\n2. Integrate CLI flag `cddm extract --bench` and MCP parameter `generate_benchmarks`.\n\n---",
    milestoneTitle: "v3.2.0 - High-Performance Parser Recycling & AST Query Memoization",
    labels: ["feat: architecture", "priority: medium"],
    closed: true,
  },
  {
    title: "[EP-37] Context-Aware Program Slicing for AI Refactor Surgeon",
    body: "- **Target Milestone**: `v2.9.0`\n- **Component**: `crates/cddm-core`\n- **Priority**: `High`\n- **Status**: `Completed (v2.9.0)`\n\n#### Problem Statement\n\nAI coding assistants need minimal yet sufficient context when synthesizing complex refactoring patches. Program slicing isolates only the relevant variable declarations and control flow paths.\n\n#### Specification & Architecture\n\n1. Implement backward and forward PDG program slicing in `cddm-core::semantic_graph::slicing`.\n2. Extract exact def-use boundary tokens for prompt injection into `cddm-core::ai_prompt`.\n\n---",
    milestoneTitle: "v3.2.0 - High-Performance Parser Recycling & AST Query Memoization",
    labels: ["feat: architecture", "priority: medium"],
    closed: true,
  },
  {
    title: "[EP-38] Ecosystem Library Reimplementation & Overlap Detector",
    body: "- **Target Milestone**: `v3.0.0`\n- **Component**: `crates/cddm-core`, `crates/cddm-cli`, `crates/cddm-mcp`, `webui`, `tui`\n- **Priority**: `High`\n- **Status**: `Completed (v3.0.0)`\n\n#### Problem Statement\n\nEngineering teams frequently reimplement standard library functions (e.g. padding, parsing, hashing, math routines) rather than leveraging battle-tested crates or npm packages.\n\n#### Specification & Architecture\n\n1. Implement `cddm-core::overlap` comparing AST token/graph structures against indexed ecosystem algorithms.\n2. Provide `cddm overlap` CLI command, MCP tool `cddm_detect_overlap`, and WebUI `OverlapDetectorModal.tsx`.\n\n---",
    milestoneTitle: "v3.2.0 - High-Performance Parser Recycling & AST Query Memoization",
    labels: ["feat: architecture", "priority: medium"],
    closed: true,
  },
  {
    title: "[EP-39] Automated GitHub PR Fix-Bot Action",
    body: "- **Target Milestone**: `v3.1.0`\n- **Component**: `ci/cd`, `crates/cddm-cli`\n- **Priority**: `Medium`\n- **Status**: `Completed (v3.1.0)`\n\n#### Problem Statement\n\nContinuous integration should offer turnkey fix suggestions and automated PR commentary when code duplication exceeds threshold policies.\n\n#### Specification & Architecture\n\n1. Publish composite GitHub Action in `.github/actions/cddm-fix-bot/action.yml`.\n2. Post markdown review comments with suggested refactoring diffs directly to the PR.\n\n---",
    milestoneTitle: "v3.2.0 - High-Performance Parser Recycling & AST Query Memoization",
    labels: ["feat: architecture", "priority: medium"],
    closed: true,
  },
  {
    title: "[EP-40] High-Performance Thread-Local Parser Recycling Pool",
    body: "- **Target Milestone**: `v3.2.0`\n- **Component**: `crates/cddm-core`\n- **Priority**: `High`\n- **Status**: `Completed (v3.2.0)`\n\n#### Problem Statement\n\nRepeated instantiation of Tree-sitter parsers per thread in large-scale multi-threaded scans creates unnecessary heap allocation overhead.\n\n#### Specification & Architecture\n\n1. Implement thread-local parser cache in `crates/cddm-core/src/ast/parser.rs`.\n2. Reuse parser instances across all files in worker thread loops.\n\n---",
    milestoneTitle: "v3.2.0 - High-Performance Parser Recycling & AST Query Memoization",
    labels: ["feat: architecture", "priority: medium"],
    closed: true,
  },
  {
    title: "[EP-42] Incremental AST Query Memoization Engine",
    body: "- **Target Milestone**: `v3.2.0`\n- **Component**: `crates/cddm-core`\n- **Priority**: `Medium`\n- **Status**: `Completed (v3.2.0)`\n\n#### Problem Statement\n\nSubtree hashing queries on unmodified files can be reused across repeat scans and incremental diff passes.\n\n#### Specification & Architecture\n\n1. Implement in-memory AST query memoization in `crates/cddm-core/src/query/memo.rs`.\n2. Invalidate memo entries based on Blake3 file hashes.\n\n---",
    milestoneTitle: "v3.2.0 - High-Performance Parser Recycling & AST Query Memoization",
    labels: ["feat: architecture", "priority: medium"],
    closed: true,
  },
  {
    title: "[EP-44] Model Context Protocol 2026 Structured Capability Annotations",
    body: "- **Target Milestone**: `v3.2.0`\n- **Component**: `crates/cddm-mcp`\n- **Priority**: `Medium`\n- **Status**: `Completed (v3.2.0)`\n\n#### Problem Statement\n\nMCP clients require structured annotations (read-only, mutating, requires-confirmation) for advanced agent safety and sandboxing.\n\n#### Specification & Architecture\n\n1. Add JSON Schema capability tags to all 31 MCP tool definitions in `crates/cddm-mcp/src/tools/`.\n\n---",
    milestoneTitle: "v3.2.0 - High-Performance Parser Recycling & AST Query Memoization",
    labels: ["feat: architecture", "priority: medium"],
    closed: true,
  },
  {
    title: "[EP-47] WebUI Treemap & Layout Hook Decomposition",
    body: "- **Target Milestone**: `v3.2.0`\n- **Component**: `webui`\n- **Priority**: `Medium`\n- **Status**: `Completed (v3.2.0)`\n\n#### Problem Statement\n\nThe WebUI Treemap component exceeded modularity limits due to coupled squarified layout math and rendering logic.\n\n#### Specification & Architecture\n\n1. Extract layout calculation to `webui/src/hooks/useTreemapLayout.ts` and `webui/src/utils/treemap-layout.ts`.\n2. Retain pure React component for canvas/SVG rendering.\n\n---",
    milestoneTitle: "v3.2.0 - High-Performance Parser Recycling & AST Query Memoization",
    labels: ["feat: architecture", "priority: medium"],
    closed: true,
  },
  {
    title: "[EP-45] Automated Dead Clone Cluster Pruning & Safe Deletion Synthesizer",
    body: "- **Target Milestone**: `v1.11.0`\n- **Component**: `crates/cddm-core`, `crates/cddm-cli`, `crates/cddm-mcp`, `webui`\n- **Priority**: `High`\n- **Status**: `Planned (v1.11.0)`\n\n#### Problem Statement\n\nWhen analyzing large codebases, clone clusters often contain functions or methods that are unreachable or unreferenced in production runtime (dead code). Manually verifying if all instances within a clone cluster can be safely removed or consolidated requires static reachability analysis.\n\n#### Specification & Architecture\n\n1. Implement `cddm_core::dead_code::prune_clusters` combining Program Dependence Graphs (PDG) and call-graph reachability.\n2. Add CLI command `cddm prune [DIR] [--dry-run] [--safe-only]` in `cddm-cli`.\n3. Expose MCP tool `cddm_prune_dead_clones` and Axum endpoint `POST /api/dead-code/prune`.\n4. Implement interactive batch deletion and safe review modal in WebUI Studio `DeadCodeExplorerModal.tsx`.\n\n---",
    milestoneTitle: "v3.3.0 - Dead Clone Elimination & Graph Reachability Engine",
    labels: ["feat: architecture", "priority: medium"],
    closed: false,
  },
  {
    title: "[EP-46] Cross-Repository Dead Code & Call-Graph Reachability Tracer",
    body: "- **Target Milestone**: `v1.11.0`\n- **Component**: `crates/cddm-core`, `webui`\n- **Priority**: `Medium`\n- **Status**: `Planned (v1.11.0)`\n\n#### Problem Statement\n\nIn multi-package monorepos, dead code may appear unused in package A but exported to package B. Tracing call-graph reachability across package boundaries is required before declaring a clone dead.\n\n#### Specification & Architecture\n\n1. Build cross-package export-import reachability graph in `cddm-core::dead_code::reachability`.\n2. Map symbol dependencies across Cargo workspaces, npm workspaces, and Go modules.\n3. Surface transitive callers in WebUI `DeadCodeExplorerModal.tsx` and TUI Dead Code view.\n\n---",
    milestoneTitle: "v3.3.0 - Dead Clone Elimination & Graph Reachability Engine",
    labels: ["feat: architecture", "priority: medium"],
    closed: false,
  },
  {
    title: "[EP-48] Subword Vector Embeddings & Neural Cosine Similarity for Polyglot Clones",
    body: "- **Target Milestone**: `v1.12.0`\n- **Component**: `crates/cddm-core`, `crates/cddm-cli`, `crates/cddm-mcp`\n- **Priority**: `High`\n- **Status**: `Planned (v1.12.0)`\n\n#### Problem Statement\n\nDetecting Type-4 semantic clones across disparate programming languages (e.g., a Rust algorithm vs a Python implementation) requires structural graph kernels combined with dense token/subword neural vector embeddings.\n\n#### Specification & Architecture\n\n1. Implement lightweight subword vector embedding generator in pure Rust (`cddm-core::semantic_graph::embedding`).\n2. Compute dense cosine similarity across AST semantic node representations.\n3. Add CLI flags `cddm semantic --neural --similarity-threshold <0.0-1.0>` in `cddm-cli`.\n4. Expose MCP tool `cddm_semantic_neural_scan` and WebUI neural clone explorer.\n\n---",
    milestoneTitle: "v3.4.0 - Semantic Neural Vector Embeddings & Polyglot Graph Alignment",
    labels: ["feat: architecture", "priority: medium"],
    closed: false,
  },
  {
    title: "[EP-49] Interactive 3D/2D Topological Semantic Neural Clone Visualizer in WebUI",
    body: "- **Target Milestone**: `v1.12.0`\n- **Component**: `webui`, `crates/cddm-cli`\n- **Priority**: `Medium`\n- **Status**: `Planned (v1.12.0)`\n\n#### Problem Statement\n\nDevelopers investigating high-dimensional semantic clones need a visual topological map (e.g., UMAP/t-SNE or force-directed graph) showing clusters of semantic similarity across modules.\n\n#### Specification & Architecture\n\n1. Implement hardware-accelerated WebGL / Canvas 2D force-directed semantic graph visualizer in `SemanticGraphModal.tsx`.\n2. Color-code nodes by programming language and cluster IDs.\n3. Support interactive node selection, line range previews, and refactor launch directly from the graph.\n\n---",
    milestoneTitle: "v3.4.0 - Semantic Neural Vector Embeddings & Polyglot Graph Alignment",
    labels: ["feat: architecture", "priority: medium"],
    closed: false,
  },
  {
    title: "[EP-50] Federation Hub Remote Peering Protocol & Distributed Cache Sync",
    body: "- **Target Milestone**: `v2.0.0`\n- **Component**: `crates/cddm-core`, `crates/cddm-cli`, `crates/cddm-mcp`\n- **Priority**: `High`\n- **Status**: `Planned (v2.0.0)`\n\n#### Problem Statement\n\nEnterprise organizations with hundreds of repositories need to discover cross-repo duplication without cloning all source code into a single local directory.\n\n#### Specification & Architecture\n\n1. Implement `cddm hub sync` client-server protocol over gRPC / HTTPS for fingerprint exchanges.\n2. Implement cryptographic privacy-preserving fingerprint hashing so proprietary code bodies are never transmitted to the hub.\n3. Expose MCP tool `cddm_sync_hub` and WebUI `HubFederationModal.tsx` remote sync controls.\n\n---",
    milestoneTitle: "v4.0.0 - Distributed Monorepo Clone Federation & AVX-512 SIMD Engine",
    labels: ["feat: architecture", "priority: medium"],
    closed: false,
  },
  {
    title: "[EP-51] AVX-512 SIMD Vector Acceleration & Lock-Free Parallel Clone Merger",
    body: "- **Target Milestone**: `v2.0.0`\n- **Component**: `crates/cddm-core`\n- **Priority**: `High`\n- **Status**: `Planned (v2.0.0)`\n\n#### Problem Statement\n\nScaling CDDM to 50M+ lines of code in enterprise monorepos requires next-generation SIMD vector lanes and lock-free concurrent hash maps.\n\n#### Specification & Architecture\n\n1. Implement AVX-512 vector lanes for Mersenne 61 rolling hash computation (`fast_mod_m61_avx512`).\n2. Replace standard index lock structures with lock-free concurrent skip-lists / partitioned Swiss tables.\n3. Add micro-benchmarks in `crates/cddm-core/benches/` validating 50M+ tokens/sec throughput.\n\n---",
    milestoneTitle: "v4.0.0 - Distributed Monorepo Clone Federation & AVX-512 SIMD Engine",
    labels: ["feat: architecture", "priority: medium"],
    closed: false,
  },
  {
    title:
      "feat(ai): replace blocking curl subprocess with async reqwest client and connection pooling",
    body: '### Problem Statement\nIn `crates/cddm-core/src/ai/provider.rs`, AI provider HTTP calls (Gemini, Claude, OpenAI, Ollama) spawn an external `curl` process via `std::process::Command::new("curl")`. This incurs heavy OS process spawning overhead, prevents HTTP/2 connection pooling, fails in containers without curl, and blocks threads.\n\n### Requirements\n1. Add `reqwest` with `json` and `stream` features to `crates/cddm-core`.\n2. Refactor `AiProvider` implementations (Ollama, Gemini, Claude, OpenAI) to use a shared async `reqwest::Client` with connection pooling.\n3. Implement proper HTTP timeouts, retry backoff, and secure proxy support.\n4. Preserve 100% hermetic isolation in `MockAiProvider` for offline unit testing.',
    milestoneTitle: "v4.2.0 - AI Harness Async Engine & Modern LLM Provider Streaming",
    labels: ["feat: ai-surgeon", "area/ai"],
    closed: false,
  },
  {
    title: "feat(ai): support streaming SSE token diff generation for AI refactor surgeon",
    body: "### Problem Statement\nRefactoring large duplicate clone clusters can take 10-30 seconds of LLM generation time. Without streaming, the CLI, WebUI, and MCP tools freeze until the complete patch arrives.\n\n### Requirements\n1. Implement asynchronous token streaming in `AiProvider::stream_prompt` returning `tokio_stream::Stream`.\n2. Stream diff hunks incrementally through Axum Server-Sent Events (SSE) in `cddm-cli::serve`.\n3. Surface live streaming diffs in WebUI Studio `RefactorPatchModal.tsx` and Monaco editor.\n4. Provide headless terminal streaming in `cddm refactor` CLI.",
    milestoneTitle: "v4.2.0 - AI Harness Async Engine & Modern LLM Provider Streaming",
    labels: ["feat: ai-surgeon", "feat: webui-studio"],
    closed: false,
  },
  {
    title: "feat(ai): update AI model catalog to Gemini 3.8 and Claude 3.7 / 4.0",
    body: "### Problem Statement\nDefault model identifiers in `crates/cddm-core/src/ai/constants.rs` are pinned to legacy generations (`gemini-2.5-pro`, `claude-3-7-sonnet`, `gpt-4.5-preview`).\n\n### Requirements\n1. Update default models to modern generations (`gemini-3.8-pro`, `claude-3-7-sonnet` / `claude-4`, `gpt-4o`).\n2. Add dynamic environment variable overrides (`CDDM_AI_MODEL`, `CDDM_AI_PROVIDER`).\n3. Support OpenAI-compatible custom endpoints (vLLM, LocalAI, LM Studio, Ollama).",
    milestoneTitle: "v4.2.0 - AI Harness Async Engine & Modern LLM Provider Streaming",
    labels: ["feat: ai-surgeon", "area/ai"],
    closed: false,
  },
  {
    title: "feat(mcp): upgrade protocol specification to 2026-07-28 and implement Tasks framework",
    body: "### Problem Statement\nCDDM's MCP server is pinned to protocol version `2024-11-05`. The modern 2026-07-28 specification formalizes stateless request/response routing, deprecates legacy roots/sampling, and introduces asynchronous **Tasks** for long-running workflows.\n\n### Requirements\n1. Update `crates/cddm-mcp/src/protocol.rs` to MCP specification version `2026-07-28`.\n2. Implement `tasks/list`, `tasks/call`, `tasks/status`, `tasks/cancel` for long monorepo scans.\n3. Add header-based routing (`Mcp-Method`, `Mcp-Name`) and caching annotations (`ttlMs`, `cacheScope`).\n4. Remove deprecated roots and sampling protocol stubs.",
    milestoneTitle: "v4.3.0 - Model Context Protocol 2026-07-28 & Background Tasks",
    labels: ["feat: mcp-protocol"],
    closed: false,
  },
  {
    title: "feat(mcp): implement MCP Apps capability for interactive UI refactoring widgets",
    body: "### Problem Statement\nAI agents refactoring code currently receive raw text patches or markdown strings. The 2026-07-28 MCP Apps extension standard allows servers to return interactive HTML widgets.\n\n### Requirements\n1. Implement MCP Apps capability handshake in `cddm-mcp`.\n2. Expose generative UI widgets (diff split-views, cluster treemaps) directly through MCP tool responses.\n3. Integrate with chat client UI hosts and standalone WebUI preview panels.",
    milestoneTitle: "v4.3.0 - Model Context Protocol 2026-07-28 & Background Tasks",
    labels: ["feat: mcp-protocol", "feat: webui-studio"],
    closed: false,
  },
  {
    title:
      "feat(telemetry): add tracing-opentelemetry distributed tracing exporter for CI pipelines",
    body: "### Problem Statement\nWhen running CDDM on multi-gigabyte enterprise repositories in CI, diagnosing whether bottlenecks originate in Tree-sitter parsing, Rayon sliding windows, or disk I/O is difficult without distributed tracing.\n\n### Requirements\n1. Integrate `tracing-opentelemetry` and `opentelemetry-otlp` into `crates/cddm-core`.\n2. Support standard environment variables (`OTEL_EXPORTER_OTLP_ENDPOINT`, `OTEL_SERVICE_NAME`).\n3. Emit structured span telemetry for: discovery, tokenization, AST parsing, winnowing, and clustering.",
    milestoneTitle: "v4.4.0 - Enterprise Observability & Developer Ergonomics",
    labels: ["ci/cd: gitea-actions", "perf: simd-optimization"],
    closed: false,
  },
  {
    title: "feat(telemetry): expose Prometheus /metrics endpoint on Axum WebUI server",
    body: "### Problem Statement\nContinuous integration daemons and production CDDM instances running `cddm serve` have no telemetry export for Prometheus, Grafana, or Netdata.\n\n### Requirements\n1. Add `metrics` and `metrics-exporter-prometheus` to `cddm-cli`.\n2. Expose `/metrics` on the Axum HTTP server.\n3. Track: active clone counts, DRY health score gauges, scan duration histograms, and SSE client counts.",
    milestoneTitle: "v4.4.0 - Enterprise Observability & Developer Ergonomics",
    labels: ["feat: webui-studio", "feat: 4-pillar-parity"],
    closed: false,
  },
  {
    title:
      "feat(cli): add shell completion generator for bash, zsh, fish, and powershell via clap_complete",
    body: "### Problem Statement\nWith 23 subcommands and dozens of flags, developers have to consult documentation or `--help` manually because CDDM lacks dynamic shell auto-completions.\n\n### Requirements\n1. Add `clap_complete` to `crates/cddm-cli/Cargo.toml`.\n2. Implement subcommand `cddm completions <SHELL>` (bash, zsh, fish, powershell, elvish).\n3. Automatically generate shell completion files during release packaging (`scripts/package-distribution.ts`).",
    milestoneTitle: "v4.4.0 - Enterprise Observability & Developer Ergonomics",
    labels: ["feat: 4-pillar-parity"],
    closed: false,
  },
  {
    title:
      "refactor(vcs): migrate workspace VCS interactions and rules to Forge MCP and deprecate custom portal scripts",
    body: "### Summary\nMigrate all CDDM workspace VCS and infrastructure interactions away from ad-hoc scripts (e.g. `scripts/populate-gitea-portal.ts`, raw curl scripts) to standardized `forge-mcp` tools and `forge` CLI commands, per organizational guidelines.\n\n### Requirements\n1. Install and enforce `.agents/rules/forge_mcp.md` via `forge_migrate_workspace`.\n2. Update governance rules (`.agents/rules/gitea-ssot-mirror-governance.md`, `.agents/rules/vcs-project-management.md`, `.agents/rules/protected-branch-pr-enforcement.md`, `AGENTS.md`) and task skills to mandate `forge-mcp` tools for issues, PRs, CI gates, and upstream defect escalation.\n3. Deprecate legacy ad-hoc portal population scripts and document Forge MCP replacements.\n4. Report all upstream feature requests and defects to `gt-dev/forge` SSoT (Issues #42, #43, #44, #45).\n5. Ensure 100% compliance across all 20 verification checks in `vp run verify` with zero CI gate bypasses.",
    milestoneTitle: "v4.2.0 - AI Harness Async Engine & Modern LLM Provider Streaming",
    labels: ["feat: architecture", "priority: high"],
    closed: false,
  },
];
